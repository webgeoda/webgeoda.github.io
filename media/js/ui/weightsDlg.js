      ...
      buttons: [{
        ...
        click: function() {
          ...
          if ( w_id.length == 0 || $('#img-id-nochk').is(":visible") ) {
            Utils.ShowMsgBox("Error", "An unique ID for weights creation is required.");
            return;
          }
          ...
          $.post("../create_weights/", post_param, function(){})
            ...
            .fail(function() { Utils.ShowMsgBox("Error", "Create weights failed."); })
            .always(function(){
    ...
    return {
      ...
      UpdateFields : function(fields) {
        ...
        if (cur_min == cur_max) {
          var mapCanvas = MapManager.getInstance().GetMapCanvas();
          $.get('../get_minmaxdist/', {
            'layer_uuid' : mapCanvas.map.uuid,
          }).done(function(result){
              //result.min max step
              $('#dist-slider').slider('option', 'min', result.min);
              $('#dist-slider').slider('option', 'max', result.max);
              $('#txt-dist-thr').val(result.min);
          });
        }
      },
