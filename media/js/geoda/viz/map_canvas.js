...
MapCanvas.prototype = {
  ...
  highlight: function( ids, ctx, nolinking ) {
    ...
    if ( ctx == undefined) {
      var hlctx = this.hlcanvas.getContext("2d");
      hlctx.clearRect(0, 0, this.canvas.width, this.canvas.height);
  ...
  triggerLink: function(select_ids, highlight_range) {
    ...
    hl[this.map.uuid] = select_ids;//this.selected;
    ...
    hl_map[this.map.uuid] = highlight_range;
    localStorage["HL_MAP"] = JSON.stringify(hl_map);

    // Now trigger inner divs
